...
public struct Row: Hashable, Equatable {
    ...
    #if os(OSX)
        public init(text: String? = nil, selection: Selection? = nil, image: Image? = nil,
                    cellClass: CellType.Type? = nil, context: Context? = nil, UUID: String = NSUUID().UUIDString) {
            self.UUID = UUID
            self.text = text
            self.image = image
            self.selection = selection
            self.cellClass = cellClass ?? PlainCell.self
            self.context = context
        }
    #else
        public init(text: String? = nil, detailText: String? = nil, selection: Selection? = nil,
                    image: Image? = nil, accessory: Accessory = .None, cellClass: CellType.Type? = nil, context: Context? = nil, editActions: [EditAction] = [], UUID: String = NSUUID().UUIDString) {
            self.UUID = UUID
            self.text = text
